...
fn count_nodes(t: &serde_json::Value) -> usize {
    let mut c = 1;
    for k in &["a", "b", "child"] {
        if let Some(ch) = t.get(k) {
            c += count_nodes(ch);
        }
    }
    if let Some(arr) = t.get("children").and_then(|v| v.as_array()) {
        for ch in arr {
            c += count_nodes(ch);
        }
    }
...
fn collect_types(t: &serde_json::Value) -> Vec<String> {
    let mut v = Vec::new();
    if let Some(s) = t.get("type").and_then(|x| x.as_str()) {
        v.push(s.into());
    }
    for k in &["a", "b", "child"] {
        if let Some(ch) = t.get(k) {
            v.extend(collect_types(ch));
        }
    }
    if let Some(arr) = t.get("children").and_then(|x| x.as_array()) {
        for ch in arr {
            v.extend(collect_types(ch));
        }
    }
...
async fn validate(Json(r): Json<ValidateReq>) -> Json<ValidateResp> {
    let nc = count_nodes(&r.tree);
    let d = tree_depth(&r.tree);
    let nt = collect_types(&r.tree);
    let mut errs = Vec::new();
    if r.tree.get("type").is_none() {
        errs.push("Root missing 'type'".into());
    }
    Json(ValidateResp {
        valid: errs.is_empty(),
        node_count: nc,
...
async fn compile(Json(r): Json<CompileReq>) -> Result<Json<CompileResp>, (StatusCode, Json<Err>)> {
    ...
    Ok(Json(CompileResp {
        success: true,
        node_count: count_nodes(&r.tree),
        depth: tree_depth(&r.tree),
        compile_time_ms: st.elapsed().as_secs_f64() * 1000.0,
    }))
}
...
async fn eval(Json(r): Json<EvalReq>) -> Result<Json<EvalResp>, (StatusCode, Json<Err>)> {
    ...
    let dists: Vec<f32> = match nt {
        "Sphere" => {
            let rad = params.get("radius").and_then(|v| v.as_f64()).unwrap_or(1.0) as f32;
            r.points
                .iter()
                .map(|p| (p[0] * p[0] + p[1] * p[1] + p[2] * p[2]).sqrt() - rad)
                .collect()
        }
        "Box3d" => {
            let h = [0.5f32, 0.5, 0.5];
            r.points
                .iter()
                .map(|p| {
    ...
    Ok(Json(EvalResp {
        point_count: dists.len(),
        distances: dists,
        eval_time_ms: elapsed.as_secs_f64() * 1000.0,
        mode: r.mode,
    }))
}
...
async fn mesh_generate(Json(r): Json<MeshReq>) -> Result<Json<MeshResp>, (StatusCode, Json<Err>)> {
    ...
    for i in 0..=res {
        for j in 0..=res {
            ...
            verts.push((
                rad * phi.sin() * theta.cos(),
                rad * phi.cos(),
                rad * phi.sin() * theta.sin(),
            ));
        }
    }
    for i in 0..res {
    ...
    Ok(Json(MeshResp {
        ...
        face_count: fc,
        format: r.format,
        generation_time_ms: st.elapsed().as_secs_f64() * 1000.0,
        data_text: obj,
    }))
}

async fn shader_transpile(
    Json(r): Json<ShaderReq>,
) -> Result<Json<ShaderResp>, (StatusCode, Json<Err>)> {
    let st = Instant::now();
    let nt = r
        .tree
        .get("type")
    ...
    Ok(Json(ShaderResp {
        target: r.target,
        source: src,
        transpile_time_ms: st.elapsed().as_secs_f64() * 1000.0,
    }))
}
...
struct PrimsResp {
    total: usize,
    primitives: Vec<PrimInfo>,
    operations: Vec<PrimInfo>,
    transforms: Vec<PrimInfo>,
    modifiers: Vec<PrimInfo>,
}

async fn list_primitives() -> Json<PrimsResp> {
    let p = |n: &str, c: &str| PrimInfo {
        name: n.into(),
        category: c.into(),
    };
    let prims = vec![
        ...
        p("Capsule", "basic"),
        p("Cone", "basic"),
        p("RoundedBox", "extended"),
        p("Ellipsoid", "extended"),
        p("Pyramid", "extended"),
        p("Octahedron", "platonic"),
        p("Tetrahedron", "platonic"),
        p("Gyroid", "tpms"),
        p("SchwarzP", "tpms"),
        p("Diamond", "tpms"),
    ];
    let ops = vec![
        p("Union", "standard"),
        p("Intersection", "standard"),
        p("Subtraction", "standard"),
        p("SmoothUnion", "smooth"),
        p("SmoothIntersection", "smooth"),
        p("SmoothSubtraction", "smooth"),
        p("ChamferUnion", "chamfer"),
        p("Xor", "special"),
        p("Morph", "special"),
    ];
    let trans = vec![
        p("Translate", "spatial"),
        p("RotateEuler", "spatial"),
        p("Scale", "spatial"),
        p("ScaleNonUniform", "spatial"),
    ];
    let mods = vec![
        p("Twist", "deform"),
        p("Bend", "deform"),
        p("Repeat", "pattern"),
        p("RepeatFinite", "pattern"),
        p("Mirror", "pattern"),
        p("PolarRepeat", "pattern"),
        p("Noise", "surface"),
        p("Shell", "surface"),
        p("Onion", "surface"),
    ];
