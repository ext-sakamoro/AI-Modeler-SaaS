...
async fn main() {
    ...
    let state = Arc::new(AppState {
        sdf_url: env("SDF_ENGINE_URL", "http://sdf-engine:8081"),
        ai_url: env("AI_LLM_URL", "http://ai-llm:8082"),
        collab_url: env("COLLAB_URL", "http://collab:8083"),
        asset_url: env("ASSET_URL", "http://asset:8084"),
    ...
    let api = Router::new()
        .route("/api/v1/sdf/*p", any(proxy_sdf))
        .route("/api/v1/mesh/*p", any(proxy_sdf))
        .route("/api/v1/shader/*p", any(proxy_sdf))
        .route("/api/v1/export", any(proxy_sdf))
        .route("/api/v1/primitives", any(proxy_sdf))
        .route("/api/v1/ai/*p", any(proxy_ai))
        .route("/api/v1/collab/*p", any(proxy_collab))
        .route("/api/v1/asset/*p", any(proxy_asset))
...
async fn license_handler() -> (HeaderMap, Json<LicenseInfo>) {
    ...
    (
        ...
        Json(LicenseInfo {
            ...
            source_code: "https://github.com/your-org/ai-modeler-saas".into(),
...
async fn proxy_sdf(
    ...
    forward(&s.sdf_url, req).await
}
async fn proxy_ai(
    State(s): State<Arc<AppState>>,
    req: Request,
) -> Result<Response, (StatusCode, Json<Err>)> {
    forward(&s.ai_url, req).await
}
async fn proxy_collab(
    State(s): State<Arc<AppState>>,
    req: Request,
) -> Result<Response, (StatusCode, Json<Err>)> {
    forward(&s.collab_url, req).await
}
async fn proxy_asset(
    State(s): State<Arc<AppState>>,
    req: Request,
) -> Result<Response, (StatusCode, Json<Err>)> {
    forward(&s.asset_url, req).await
}
